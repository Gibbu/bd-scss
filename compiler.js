 ...
 * @param {boolean} options.meta Tells the compiler to include the META at the top of the file.
 * @return {Promise<void>}
 */
export default async (options) => {
	const startTime = performance.now().toFixed();
	...
	const generatedFile = options.meta ? constructMeta() + CSS : CSS;

	const endTime = performance.now().toFixed();
	...
	try {
		...
		console.log(
			chalk.greenBright.bold('[SUCCESS]') +
				...
				' to ' +
				chalk.yellow(`\`${options.output.join(osSlash)}\``) +
				' in ' +
				chalk.cyanBright(endTime - startTime + 'ms')
		);
	} catch (error) {
